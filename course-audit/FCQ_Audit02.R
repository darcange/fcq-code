...
# figure out why some indGiveOnlyLEC have 1s
# do i need to update BD-EV totLECInstr to 0?
# should BD-MCEN senior design exceptions be added to indGiveOnlyLEC?
...
n03 <- n02 %>%
  mutate(indDEPT_RQST = case_when(
    # BD-IPHY 
    crseSec_comp_cd %in% c("INT", "PRA") & deptOrgID == "CUBLD:BLDR:IPHY" ~ 1,
    # BD-GEEN (2018/10/30 Ellen Parrish) courses get FCQs for LEC ONLY, not LAB
    ...
    # BD-MCEN (2020/03/23 Vanessa Dunn) senior design crse get FCQs for LEC ONLY
    crseSec_comp_cd != "LEC" & deptOrgID == "CUBLD:BLDR:MCEN" & 
      CATALOG_NBR %in% c("4045", "4085") ~ 1,
    # (2018/11/01 Deb Prestianni) no FCQs for all sections of 7116 and 7939
...
# indCrseTooLate
# classes too late in term to be administered FCQs excluding Beijing classes
library("anytime")
...
# indBeijing
# not active as of 4/7/22, but may be relevant in the future
# n?? <- n?? %>%
#   mutate(indBeijing = case_when(
#     INSTITUTION_CD == "CUDEN" & LOC_ID == "IC_BEIJING" ~ 1,
#     TRUE ~ 0
#   ))
...
# INSTITUTION_CD == "CUDEN" & indBeijing == 1 ~
#    "No Beijing FCQs per dept req"

# create fcqNote
n14$fcqNote <-apply(n14[,87:ncol(n14)],1,function(x) toString(na.omit(x)))
